const solve = (input) => {  
  ...
  for(const rule of rules){
  	const bag = toBagRule(rule);
    bagMap.set(bag.name, bag.contains);
  }
  
  let canHoldGold = 0;
  bagMap.forEach((_, name) => canHoldGold += canHold(name, bagMap, 'shiny gold') ? 1 : 0);
  
  const goldBagsContain = containsBagCount('shiny gold', bagMap);
  
  console.log(`Part1: ${canHoldGold}`);     
  console.log(`Part2: ${goldBagsContain}`);   
}

const canHold = (name, bagMap, bagToHold) => {
  const contains = bagMap.get(name);
  for(const bag of contains){
  	if (bag.name === bagToHold || canHold(bag.name, bagMap, bagToHold)){
    	return true;
    }
  }
  return false;
...
const containsBagCount = (name, bagMap) => {
  const contains = bagMap.get(name);
  let count = 0;
  for(const bag of contains){
  	count += bag.count + bag.count * containsBagCount(bag.name, bagMap) ;
  }
  return count;
}

const toBagRule = (rule) => {
  const bagReg = /([\w\s]+) bags contain/;
  const containReg = /(\d)\s([\w\s]+) bag/g;
  
  const bag = {
    name:rule.match(bagReg)[1],
    contains:[]
  }

  let contains;
  while ((contains = containReg.exec(rule)) !== null) {
    bag.contains.push({
      name:contains[2],
      count:Number(contains[1])
    });
  }
  return bag;
}

solve(`dotted blue bags contain 3 wavy bronze bags, 5 clear tomato bags.
