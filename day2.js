...
const countValid = (input) => {
  ...
  for(const pw of passwords){
   const [, a, b, character, password] = pw.match(reg);
 
   const count = password.split('').reduce((c, x) => c + (x==character?1:0), 0); 
   if (count >= a && count <= b){ p1valid++; }
 
   const oneIsChar = password[Number(a)-1] === character;
   const twoIsChar =  password[Number(b)-1] === character;
   if (oneIsChar != twoIsChar){ p2valid++; }
  }
  
  console.log(`Part1: ${p1valid}`);
  console.log(`Part2: ${p2valid}`);
}

countValid(`6-10 p: ctpppjmdpppppp
